...
public class RBindCommand implements CommandExecutor {
    public static void setBindCombination(@NotNull Player player, List<Boolean> combination) {
        NamespacedKey key = new NamespacedKey(MCWizards.getPlugin(MCWizards.class), "BoundToR");
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        if (sender instanceof Player player) {
            if (args.length == 1) {
                ...
                ((Player) sender).sendMessage("Bound combination R to " + RegisterSpells.getSpell(combination).name);
            }
        }
        return true;
    }
